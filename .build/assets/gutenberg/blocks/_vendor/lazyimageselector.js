...
import { getLazySrcs } from './lazyimage.js';
...
export default class LazyImageSelector extends Component {
	...
	render() {
		const { allowedTypes, attributes, attributeKey, imageFormat, setAttributes, objectFocalPoint } = this.props;
				...
				<div className='c-imageselector'>
					<MediaUploadCheck>
						<MediaUpload
							...
							render={({ open }) => {
								return (
									<figure className={`c-imageselector__figure ${!image_attribute.id ? 'c-imageselector__figure--noimage' : ''}`} style={style_orig}>
										{!!image_attribute.id && <img className='c-imageselector__image' onClick={open} src={image_attribute.org[0]} alt={image_attribute.alt} style={style_orig} />}
										<div className='c-imageselector__buttons'>
											...
											{!!image_attribute.id && (
												<Fragment>
													...
													<Button onClick={() => setAttributes({ [attribute_key]: { id: false } })} isSmall isSecondary>
