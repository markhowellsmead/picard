	...
	save({ attributes }) {
		...
		return (
			<section className={`${className} ${attributes.ratio}`}>
				{!!attributes.image.id && <LazyImage className={`${classNameBase}__figure`} image={attributes.image} background={false} admin={false} objectFocalPoint={attributes.focalPoint} />}
				{!!attributes.figcaption && <RichText.Content style={textStyle} tagName='figcaption' className={`${classNameBase}__figcaption`} value={attributes.figcaption} />}
			</section>
		);
	},
});
