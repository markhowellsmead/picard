				...
				{imageData && imageData.media_details && (
					<InspectorControls>
						...
						<PanelBody title={_x('Colours', 'Domain Gutenberg Block Panel Title', 'sha')} initialOpen={true}>
							...
							<ColorPalette colors={colors} value={attributes.textColor} onChange={textColor => setAttributes({ textColor })} />
							<RangeControl label={_x('Text transparency', 'Range control label', 'sha')} value={attributes.textOpacity} onChange={textOpacity => setAttributes({ textOpacity })} min={0} max={100} />
						</PanelBody>
					</InspectorControls>
				)}
				<section className={`${className} ${attributes.ratio}`}>
					<div className={`${classNameBase}__inner`}>
						<div className={`${classNameBase}__content`}>
							<InnerBlocks
								allowedBlocks={(['core/heading'], ['core/paragraph'], ['mhm/viewpoint-ancestors'])}
								template={[
									[
										'core/heading',
										{
						...
						<div className={`${classNameBase}__figurewrap ${attributes.ratio}`}>
							<LazyImageSelector attributes={attributes} className={`${classNameBase}__figure`} image={attributes.image} setAttributes={setAttributes} objectFocalPoint={attributes.focalPoint} admin={true} imageFormat={imageFormat} />
							<RichText
								style={textStyle}
								tagName='figcaption'
