...
export default class ImageSelectorWithPlaceholder extends Component {
	constructor(props) {
		...
		const { attributes, setAttributes, imageAttribute, imageExternalURLAttribute, imageFormat, allowURL, accept, allowedTypes, labels, labelReplace } = this.props;
		...
		this.label_replace = this.labels_object.replace || _x('Bild ersetzen', 'MediaFlow replace label', 'sha');
		...
		return (
			<Fragment>
				{(!!image.id || !!imageExternalURL) && !this.state.is_uploading && (
					<BlockControls>
						<Toolbar>
							<MediaReplaceFlow name={this.label_replace} mediaId={image.id} mediaURL={this.imageExternalURL_attribute} accept={this.accept_types} allowedTypes={this.allowed_types} onFilesUpload={this.onPreUpload} onSelect={this.onSelect} onSelectURL={this.onSelectURL} />
						</Toolbar>
					</BlockControls>
				)}
				...
				{!image.id && !imageExternalURL && !this.state.is_uploading && (
					<div className='c-mediaselector c-mediaselector--image components-base-control components-lazy-image-selector'>
						<MediaPlaceholder icon='format-image' labels={this.labels_object} multiple={false} onFilesPreUpload={this.onPreUpload} onSelect={this.onSelect} onSelectURL={this.onSelectURL} accept={this.accept_types} allowedTypes={this.allowed_types} />
					</div>
				)}
			</Fragment>
		);
