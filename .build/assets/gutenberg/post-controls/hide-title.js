...
const PageOptionsPanel = ({ hide_title, post_type, onUpdateHideTitle }) => {
	...
	return (
		<PluginDocumentSettingPanel title={_x('Page options', 'Editor sidebar panel title', 'sht')} initialOpen={true} icon={'invalid-name-no-icon'}>
			<ToggleControl label={_x('Hide title', 'ToggleControl label', 'sha')} help={hide_title ? _x('The title is hidden for the current post. It is recommended that you add a H1 to the content in order to better support search engines.', 'Warning text', 'sha') : ''} checked={hide_title} onChange={hide_title => onUpdateHideTitle(hide_title)} />
